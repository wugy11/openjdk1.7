...
static jboolean ParseArguments(int *pargc, char ***pargv, int *pmode,
		char **pwhat, int *pret, const char *jrepath);
...
static void TranslateApplicationArgs(int jargc, const char **jargv, int *pargc,
		char ***pargv);
...
enum LaunchMode {               // cf. sun.launcher.LauncherHelper
	LM_UNKNOWN = 0, LM_CLASS, LM_JAR
};

static const char *launchModeNames[] = { "Unknown", "Main class", "JAR file" };
...
CheckJvmType(int *pargc, char ***argv, jboolean speculative) {
	...
	for (argi = 1; argi < argc; argi++) {
		...
		else if (JLI_StrCCmp(arg, "-XXaltjvm=") == 0
				|| JLI_StrCCmp(arg, "-J-XXaltjvm=") == 0) {
			isVMType = 1;
...
static void SetClassPath(const char *s) {
	...
	def = JLI_MemAlloc(sizeof(format) - 2 /* strlen("%s") */
	+ JLI_StrLen(s));
...
static void SelectVersion(int argc, char **argv, char **main_class) {
	...
	*new_argp = NULL;
	...
	if (jarflag && operand) {
		 ...
		 * line parsing
		 */
		if (!headlessflag && !splash_file_name
	...
	if (splash_file_name && !headlessflag) {
		char* splash_file_entry = JLI_MemAlloc(
				JLI_StrLen(SPLASH_FILE_ENV_ENTRY "=")
	...
	if (splash_jar_name && !headlessflag) {
		char* splash_jar_entry = JLI_MemAlloc(
				JLI_StrLen(SPLASH_JAR_ENV_ENTRY "=")
	...
	JLI_TraceLauncher(
			...
			(info.jre_restrict_search ? "true" : "false"),
			(jre ? jre : "null"));
	...
	*pret = 0;
	...
	while ((arg = *argv) != 0 && *arg == '-') {
		...
		JLI_StrCmp(arg, "-?") == 0) {
			...
			JLI_ReportMessage("%s full version \"%s\"", _launcher_name,
					GetFullVersion());
			return JNI_FALSE;
			...
			AddOption(tmp, NULL);
		} else if (JLI_StrCCmp(arg, "-ss") == 0 || JLI_StrCCmp(arg, "-oss") == 0
...
static jclass GetLauncherHelperClass(JNIEnv *env) {
	if (helperClass == NULL) {
		NULL_CHECK0(
				helperClass = FindBootStrapClass(env,
...
static jclass LoadMainClass(JNIEnv *env, int mode, char *name) {
	...
	NULL_CHECK0(
			mid = (*env)->GetStaticMethodID(env, cls, "checkAndLoadMain",
	...
	result = (*env)->CallStaticObjectMethod(env, cls, mid, USE_STDERR, mode,
			str);
	...
	for (i = 0; i < argc; i++) {
		...
		if (arg[0] == '-') {
			...
			if (IsWildCardEnabled() && arg[1] == 'c'
					&& (JLI_StrCmp(arg, "-cp") == 0 ||
					JLI_StrCmp(arg, "-classpath") == 0) && i < argc - 1) {
				*nargv++ = arg;
...
static void PrintJavaVersion(JNIEnv *env, jboolean extraLF) {
	...
	NULL_CHECK(
			print = (*env)->GetStaticMethodID(env, ver, (extraLF == JNI_TRUE) ? "println" : "print", "()V" ));
...
static void ShowSettings(JNIEnv *env, char *optString) {
	...
	NULL_CHECK(cls);
	NULL_CHECK(
			showSettingsID = (*env)->GetStaticMethodID(env, cls, "showSettings",
					"(ZLjava/lang/String;JJJZ)V"));
	joptString = (*env)->NewStringUTF(env, optString);
	(*env)->CallStaticVoidMethod(env, cls, showSettingsID,
	USE_STDERR, joptString, (jlong) initialHeapSize, (jlong) maxHeapSize,
			(jlong) threadStackSize, ServerClassMachine());
}
 ...
 */
static void PrintUsage(JNIEnv* env, jboolean doXUsage) {
	jmethodID initHelp, vmSelect, vmSynonym, vmErgo, printHelp,
			printXUsageMessage;
	...
	if (doXUsage) {
		NULL_CHECK(
				printXUsageMessage = (*env)->GetStaticMethodID(env, cls,
						"printXUsageMessage", "(Z)V"));
		(*env)->CallStaticVoidMethod(env, cls, printXUsageMessage, USE_STDERR);
	} else {
		NULL_CHECK(
				initHelp = (*env)->GetStaticMethodID(env, cls,
						"initHelpMessage", "(Ljava/lang/String;)V"));

		NULL_CHECK(
				vmSelect = (*env)->GetStaticMethodID(env, cls,
						"appendVmSelectMessage",
						"(Ljava/lang/String;Ljava/lang/String;)V"));

		NULL_CHECK(
				vmSynonym = (*env)->GetStaticMethodID(env, cls,
						"appendVmSynonymMessage",
						"(Ljava/lang/String;Ljava/lang/String;)V"));
		NULL_CHECK(
				vmErgo = (*env)->GetStaticMethodID(env, cls,
						"appendVmErgoMessage", "(ZLjava/lang/String;)V"));

		NULL_CHECK(
				printHelp = (*env)->GetStaticMethodID(env, cls,
		...
		/* The first known VM is the default */
		{
			...
			if ((knownVMs[0].flag == VM_IF_SERVER_CLASS)
					&& isServerClassMachine) {
				defaultVM = knownVMs[0].server_class + 1;
			}

			vm1 = (*env)->NewStringUTF(env, defaultVM);
			(*env)->CallStaticVoidMethod(env, cls, vmErgo, isServerClassMachine,
					vm1);
		}
		...
		if (vmType != VM_UNKNOWN) {
			...
			case VM_ALIASED_TO:
				...
				JLI_TraceLauncher("    name: %s  vmType: %s  alias: %s\n",
						knownVMs[cnt].name, "VM_ALIASED_TO",
						knownVMs[cnt].alias);
				break;
			case VM_IF_SERVER_CLASS:
				...
				JLI_TraceLauncher(
						...
						knownVMs[cnt].name, "VM_IF_SERVER_CLASS",
						knownVMs[cnt].server_class);
				break;
			}
			cnt++;
		}
 ...
 */
jboolean RemovableOption(char * option) {
	/*
 ...
 */
void JLI_ReportMessage(const char* fmt, ...) {
	va_list vl;
	va_start(vl, fmt);
