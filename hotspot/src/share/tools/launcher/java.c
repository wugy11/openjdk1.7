...
static void SetJavaCommandLineProp(char* classname, char* jarfile, int argc,
		char** argv);
...
JavaMain(void * _args) {
	...
	if (jarfile != 0) {
		...
		if (mainClassName == NULL) {
			const char * format = "Failed to load Main-Class manifest "
					"attribute from\n%s";
			message = (char*) JLI_MemAlloc(
					(strlen(format) + strlen(jarfile)) * sizeof(char));
		...
		if (mainClass == NULL) { /* exception occured */
			...
			ReportExceptionDescription(env);
			message = (char *) JLI_MemAlloc(
					(strlen(format) + strlen(classname)) * sizeof(char));
			messageDest = JNI_TRUE;
	...
	{ /* Make sure the main method is public */
		jint mods;
		jmethodID mid;
		jobject obj = (*env)->ToReflectedMethod(env, mainClass, mainID,
				JNI_TRUE);
...
CheckJvmType(int *pargc, char ***argv, jboolean speculative) {
	...
	for (argi = 1; argi < argc; argi++) {
		...
		else if (strncmp(arg, "-XXaltjvm=", 10) == 0
				|| strncmp(arg, "-J-XXaltjvm=", 12) == 0) {
			isVMType = 1;
	...
	*pargc = newArgvIdx;
	...
	case VM_ERROR:
		if (!speculative) {
			ReportErrorMessage2("Error: %s VM not supported", jvmtype,
					JNI_TRUE);
			exit(1);
		} else {
			return "ERROR";
...
static int parse_stack_size(const char *s, jlong *result) {
	jlong n = 0;
	...
	*new_argp = NULL;
	...
	if (jarflag && operand) {
		if ((res = JLI_ParseManifest(operand, &info)) != 0) {
			if (res == -1)
				ReportErrorMessage2("Unable to access jarfile %s", operand,
						JNI_TRUE);
			else
				ReportErrorMessage2("Invalid or corrupt jarfile %s", operand,
						JNI_TRUE);
			exit(1);
		}
	...
	if (splash_file_name && !headlessflag) {
		char* splash_file_entry = JLI_MemAlloc(
				strlen(SPLASH_FILE_ENV_ENTRY "=") + strlen(splash_file_name)
						+ 1);
	...
	if (splash_jar_name && !headlessflag) {
		char* splash_jar_entry = JLI_MemAlloc(
				strlen(SPLASH_JAR_ENV_ENTRY "=") + strlen(splash_jar_name) + 1);
...
static jboolean ParseArguments(int *pargc, char ***pargv, char **pjarfile,
	...
	*pret = 1;
	while ((arg = *argv) != 0 && *arg == '-') {
		...
		if (strcmp(arg, "-classpath") == 0 || strcmp(arg, "-cp") == 0) {
			if (argc < 1) {
				ReportErrorMessage2("%s requires class path specification", arg,
						JNI_TRUE);
				PrintUsage();
				return JNI_FALSE;
			}
			...
			*pret = 0;
			...
			AddOption(tmp, NULL);
		} else if (strncmp(arg, "-ss", 3) == 0 || strncmp(arg, "-oss", 4) == 0
				|| strncmp(arg, "-ms", 3) == 0 || strncmp(arg, "-mx", 3) == 0) {
			char *tmp = JLI_MemAlloc(strlen(arg) + 6);
			sprintf(tmp, "-X%s", arg + 1); /* skip '-' */
			AddOption(tmp, NULL);
		} else if (strcmp(arg, "-checksource") == 0 || strcmp(arg, "-cs") == 0
...
static jstring getPlatformEncoding(JNIEnv *env) {
	if (platformEncoding == NULL) {
		...
		if (propname) {
			...
			platformEncoding = (*env)->CallStaticObjectMethod(env, cls, mid,
					propname);
		}
	}
...
static jboolean isEncodingSupported(JNIEnv *env, jstring enc) {
	...
	NULL_CHECK0(
			mid = (*env)->GetStaticMethodID(env, cls, "isSupported",
					"(Ljava/lang/String;)Z"));
 ...
 */
void SetJavaCommandLineProp(char *classname, char *jarfile, int argc,
		char **argv) {
		...
		*tmpPtr++ = 0;
		...
		if (*tmpPtr == 0) {
			...
			if (!strncmp(tmpPtr, "KNOWN", strlen("KNOWN"))) {
				...
				vmType = VM_ERROR;
			} else if (!strncmp(tmpPtr, "IF_SERVER_CLASS",
