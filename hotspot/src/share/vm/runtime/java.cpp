...
void print_method_invocation_histogram() {
	...
	//
	tty->cr();
	tty->print_cr("Histogram Over MethodOop Invocation Counters (cutoff = %d):",
			MethodHistogramCutoff);
	tty->cr();
	tty->print_cr(
			"____Count_(I+C)____Method________________________Module_________________");
	unsigned total = 0, int_total = 0, comp_total = 0, static_total = 0,
			final_total = 0, synch_total = 0, nativ_total = 0, acces_total = 0;
	...
	tty->print_cr("\t%9d (%4.1f%%) interpreted", int_total,
			100.0 * int_total / total);
	tty->print_cr("\t%9d (%4.1f%%) compiled", comp_total,
			100.0 * comp_total / total);
	tty->print_cr("\t%9d (100%%)  total", total);
	tty->print_cr("\t%9d (%4.1f%%) synchronized", synch_total,
			100.0 * synch_total / total);
	tty->print_cr("\t%9d (%4.1f%%) final", final_total,
			100.0 * final_total / total);
	tty->print_cr("\t%9d (%4.1f%%) static", static_total,
			100.0 * static_total / total);
	tty->print_cr("\t%9d (%4.1f%%) native", nativ_total,
			100.0 * nativ_total / total);
	tty->print_cr("\t%9d (%4.1f%%) accessor", acces_total,
			100.0 * acces_total / total);
	tty->cr();
...
void print_bytecode_count() {
	if (CountBytecodes || TraceBytecodes || StopInterpreterAt) {
		tty->print_cr("[BytecodeCounter::counter_value = %d]",
				BytecodeCounter::counter_value());
	}
}
...
HS_DTRACE_PROBE(hotspot, vm__shutdown); HS_DTRACE_WORKAROUND_TAIL_CALL_BUG();
}
...
if (func == NULL) {
	...
	_current = JDK_Version(major, minor, micro, info.update_version,
			info.special_update_version, build, info.thread_park_blocker == 1,
			info.post_vm_init_hook_enabled == 1);
}
}

void JDK_Version::fully_initialize(uint8_t major, uint8_t minor, uint8_t micro,
	uint8_t update) {
...
assert(major < 6, "not needed for JDK version >= 6"); assert(is_partially_initialized(), "must not initialize");
if (major < 5) {
...
return ((int64_t) v.major_version() << (BitsPerByte * 5))
...
if (!is_valid()) {
	...
	index += jio_snprintf(&buffer[index], buflen - index, "%d.%d", _major,
			_minor);
	if (_micro > 0) {
