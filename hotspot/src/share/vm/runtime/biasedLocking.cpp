...
static void enable_biased_locking(klassOop k) {
	Klass::cast(k)->set_prototype_header(
			markOopDesc::biased_locking_prototype());
}
...
public:
	VM_EnableBiasedLocking(bool is_cheap_allocated) {
		_is_cheap_allocated = is_cheap_allocated;
	}
	VMOp_Type type() const {
		return VMOp_EnableBiasedLocking;
	}
	Mode evaluation_mode() const {
		return _is_cheap_allocated ? _async_safepoint : _safepoint;
	}
	bool is_cheap_allocated() const {
		return _is_cheap_allocated;
	}
...
public:
	EnableBiasedLockingTask(size_t interval_time) :
...
static GrowableArray<MonitorInfo*>* get_or_compute_monitor_info(
		JavaThread* thread) {
...
static BiasedLocking::Condition revoke_bias(oop obj, bool allow_rebias,
	...
	if (!mark->has_bias_pattern()) {
		if (TraceBiasedLocking) {
			ResourceMark rm;
			tty->print_cr(
					"  (Skipping revocation of object of type %s because it's no longer biased)",
	...
	int age = mark->age();
	markOop biased_prototype = markOopDesc::biased_locking_prototype()->set_age(
			age);
	...
	if (TraceBiasedLocking && (Verbose || !is_bulk)) {
		ResourceMark rm;
		tty->print_cr(
				"Revoking bias of object " INTPTR_FORMAT " , mark " INTPTR_FORMAT " , type %s , prototype header " INTPTR_FORMAT " , allow rebias %d , requesting thread " INTPTR_FORMAT,
				(intptr_t) obj, (intptr_t) mark,
				Klass::cast(obj->klass())->external_name(),
				(intptr_t) Klass::cast(obj->klass())->prototype_header(),
				(allow_rebias ? 1 : 0), (intptr_t) requesting_thread);
	}
	...
	if (requesting_thread == biased_thread) {
		...
		for (JavaThread* cur_thread = Threads::first(); cur_thread != NULL;
	...
	for (int i = 0; i < cached_monitor_info->length(); i++) {
		...
		if (mon_info->owner() == obj) {
			if (TraceBiasedLocking && Verbose) {
				tty->print_cr(
						"   mon_info->owner (" PTR_FORMAT ") == obj (" PTR_FORMAT ")",
						(intptr_t) mon_info->owner(), (intptr_t) obj);
			}
...
static HeuristicsResult update_heuristics(oop o, bool allow_rebias) {
	...
	jlong last_bulk_revocation_time =
...
static BiasedLocking::Condition bulk_revoke_or_rebias_at_safepoint(oop o,
	...
	if (TraceBiasedLocking) {
		tty->print_cr(
				"* Beginning bulk revocation (kind == %s) because of object "
				INTPTR_FORMAT " , mark " INTPTR_FORMAT " , type %s",
				(bulk_rebias ? "rebias" : "revoke"), (intptr_t) o,
				(intptr_t) o->mark(), Klass::cast(o->klass())->external_name());
	}
	...
	if (bulk_rebias) {
		...
		if (klass->prototype_header()->has_bias_pattern()) {
			...
			for (JavaThread* thr = Threads::first(); thr != NULL;
					thr = thr->next()) {
				GrowableArray<MonitorInfo*>* cached_monitor_info =
						get_or_compute_monitor_info(thr);
				for (int i = 0; i < cached_monitor_info->length(); i++) {
					MonitorInfo* mon_info = cached_monitor_info->at(i);
					oop owner = mon_info->owner();
		...
		revoke_bias(o,
				attempt_rebias_of_object
						&& klass->prototype_header()->has_bias_pattern(), true,
				requesting_thread);
	} else {
		if (TraceBiasedLocking) {
			ResourceMark rm;
			tty->print_cr("* Disabling biased locking for type %s",
					klass->external_name());
		}
		...
		for (JavaThread* thr = Threads::first(); thr != NULL; thr =
				thr->next()) {
			GrowableArray<MonitorInfo*>* cached_monitor_info =
					get_or_compute_monitor_info(thr);
			for (int i = 0; i < cached_monitor_info->length(); i++) {
	...
	if (attempt_rebias_of_object && o->mark()->has_bias_pattern()
			&& klass->prototype_header()->has_bias_pattern()) {
		markOop new_mark = markOopDesc::encode(requesting_thread,
				o->mark()->age(), klass->prototype_header()->bias_epoch());
		o->set_mark(new_mark);
		status_code = BiasedLocking::BIAS_REVOKED_AND_REBIASED;
		if (TraceBiasedLocking) {
			tty->print_cr("  Rebiased object toward thread " INTPTR_FORMAT,
...
public:
	VM_RevokeBias(Handle* obj, JavaThread* requesting_thread) :
			_obj(obj), _objs(NULL), _requesting_thread(requesting_thread), _status_code(
					BiasedLocking::NOT_BIASED) {
	}

	VM_RevokeBias(GrowableArray<Handle>* objs, JavaThread* requesting_thread) :
			_obj(NULL), _objs(objs), _requesting_thread(requesting_thread), _status_code(
					BiasedLocking::NOT_BIASED) {
	}

	virtual VMOp_Type type() const {
		return VMOp_RevokeBias;
	}
	...
	virtual void doit() {
		if (_obj != NULL) {
			...
			_status_code = revoke_bias((*_obj)(), false, false,
					_requesting_thread);
...
public:
	VM_BulkRevokeBias(Handle* obj, JavaThread* requesting_thread,
			bool bulk_rebias, bool attempt_rebias_of_object) :
			VM_RevokeBias(obj, requesting_thread), _bulk_rebias(bulk_rebias), _attempt_rebias_of_object(
					attempt_rebias_of_object) {
	}

	virtual VMOp_Type type() const {
		return VMOp_BulkRevokeBias;
	}
	virtual bool doit_prologue() {
		return true;
	}
	...
	virtual void doit() {
		_status_code = bulk_revoke_or_rebias_at_safepoint((*_obj)(),
				_bulk_rebias, _attempt_rebias_of_object, _requesting_thread);
		clean_up_cached_monitor_info();
	}
};
...
BiasedLocking::Condition BiasedLocking::revoke_and_rebias(Handle obj,
	...
	if (mark->is_biased_anonymously() && !attempt_rebias) {
		...
		markOop unbiased_prototype = markOopDesc::prototype()->set_age(
				mark->age());
		...
		if (!prototype_header->has_bias_pattern()) {
			...
			if (attempt_rebias) {
				...
				markOop biased_value = mark;
				markOop rebiased_prototype = markOopDesc::encode(
						(JavaThread*) THREAD, mark->age(),
						prototype_header->bias_epoch());
				markOop res_mark = (markOop) Atomic::cmpxchg_ptr(
						rebiased_prototype, obj->mark_addr(), mark);
				if (res_mark == biased_value) {
				...
				markOop unbiased_prototype = markOopDesc::prototype()->set_age(
						mark->age());
				markOop res_mark = (markOop) Atomic::cmpxchg_ptr(
						unbiased_prototype, obj->mark_addr(), mark);
				if (res_mark == biased_value) {
					return BIAS_REVOKED;
				}
			}
	...
	if (heuristics == HR_NOT_BIASED) {
		...
		if (mark->biased_locker() == THREAD
			...
			BiasedLocking::Condition cond = revoke_bias(obj(), false, false,
					(JavaThread*) THREAD);
...
void BiasedLocking::revoke_at_safepoint(Handle h_obj) {
	...
	if (heuristics == HR_SINGLE_REVOKE) {
		...
		bulk_revoke_or_rebias_at_safepoint(obj, (heuristics == HR_BULK_REBIAS),
				false, NULL);
	}
	clean_up_cached_monitor_info();
}
	...
	for (int i = 0; i < len; i++) {
		...
		if (heuristics == HR_SINGLE_REVOKE) {
			revoke_bias(obj, false, false, NULL);
				...
				|| (heuristics == HR_BULK_REVOKE)) {
			bulk_revoke_or_rebias_at_safepoint(obj,
					(heuristics == HR_BULK_REBIAS), false, NULL);
		}
	}
	clean_up_cached_monitor_info();
}
...
void BiasedLocking::preserve_marks() {
	...
	assert(_preserved_oop_stack == NULL, "double initialization"); assert(_preserved_mark_stack == NULL, "double initialization");
	...
	_preserved_mark_stack = new (ResourceObj::C_HEAP) GrowableArray<markOop>(10,
			true);
	_preserved_oop_stack = new (ResourceObj::C_HEAP) GrowableArray<Handle>(10,
			true);

	ResourceMark rm;
	Thread* cur = Thread::current();
	for (JavaThread* thread = Threads::first(); thread != NULL;
			thread = thread->next()) {
		if (thread->has_last_Java_frame()) {
			RegisterMap rm(thread);
			for (javaVFrame* vf = thread->last_java_vframe(&rm); vf != NULL;
...
void BiasedLocking::restore_marks() {
	...
	assert(_preserved_oop_stack != NULL, "double free"); assert(_preserved_mark_stack != NULL, "double free");
...
int* BiasedLocking::total_entry_count_addr() {
	return _counters.total_entry_count_addr();
}
int* BiasedLocking::biased_lock_entry_count_addr() {
	return _counters.biased_lock_entry_count_addr();
}
int* BiasedLocking::anonymously_biased_lock_entry_count_addr() {
	return _counters.anonymously_biased_lock_entry_count_addr();
}
int* BiasedLocking::rebiased_lock_entry_count_addr() {
	return _counters.rebiased_lock_entry_count_addr();
}
int* BiasedLocking::revoked_lock_entry_count_addr() {
	return _counters.revoked_lock_entry_count_addr();
}
int* BiasedLocking::fast_path_entry_count_addr() {
	return _counters.fast_path_entry_count_addr();
}
int* BiasedLocking::slow_path_entry_count_addr() {
	return _counters.slow_path_entry_count_addr();
}
...
void BiasedLockingCounters::print_on(outputStream* st) {
	...
	tty->print_cr("# anonymously biased lock entries: %d",
